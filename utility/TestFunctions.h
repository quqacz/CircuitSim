...
template<typename T> void TEST(std::string testName, T output, T expectedOutput){
    if(output == expectedOutput){
        ...
        std::cout<< "\033[1;31m(fail)\t" << testName << " fail; expected: " << expectedOutput << ", provided: " << output << "\033[0m\n";
    }
}
...
template<typename T> void TEST(std::string testName, T output, T expectedOutput, int& success, int& fail){
    if(output == expectedOutput){
        std::cout<< "\033[1;32m(pass)\t" << testName << " pass \033[0m\n";
        success += 1;
    }else{
